...
ID3D11Buffer*			g_ParamBuffer = NULL;
ID3D11Buffer*			g_RandomBuffer = NULL;
ID3D11Buffer*			g_GlitchBuffer = NULL;
...
void SetRandom(RANDOM rand)
{
	...
	GetDeviceContext()->UpdateSubresource(g_RandomBuffer, 0, NULL, &rand, 0, 0);
...
void SetGlitch(GLITCH glitch)
{
	...
	GetDeviceContext()->UpdateSubresource(g_GlitchBuffer, 0, NULL, &glitch, 0, 0);
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	hBufferDesc.ByteWidth = sizeof(GLITCH);
	hBufferDesc.Usage = D3D11_USAGE_DEFAULT;
	hBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	hBufferDesc.CPUAccessFlags = 0;
	hBufferDesc.MiscFlags = 0;
	...
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_RandomBuffer);
	g_ImmediateContext->VSSetConstantBuffers(3, 1, &g_RandomBuffer);
	g_ImmediateContext->PSSetConstantBuffers(3, 1, &g_RandomBuffer);
...
void UninitRenderer(void)
{
	...
	if (g_ParamBuffer)			g_ParamBuffer->Release();
